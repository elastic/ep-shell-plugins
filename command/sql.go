...
type whereCmd struct {
	pkgsContextKey, dbContextKey plugin.CtxKey
	p                            *plugin.Plugin
	name, usage, desc            string
}

func AddWhereCmd(p *plugin.Plugin, pkgsContextKey, dbContextKey plugin.CtxKey) {
	cmd := &whereCmd{
...
func (c *whereCmd) Exec(wd string, args []string, _, stderr io.Writer) error {
	db, ok := c.p.GetValueFromCtx(c.dbContextKey).(*sql.DB)
	if !ok {
	...
	c.p.AddValueToCtx(c.pkgsContextKey, pkgs)
...
type initdbCmd struct {
	dbContextKey      plugin.CtxKey
	p                 *plugin.Plugin
	name, usage, desc string
}

func AddInitdbCmd(p *plugin.Plugin, dbContextKey plugin.CtxKey) {
	cmd := &initdbCmd{
		dbContextKey: dbContextKey,
		p:            p,
		name:         "initdb",
...
func (c *initdbCmd) Exec(wd string, args []string, _, stderr io.Writer) error {
	...
	c.p.AddValueToCtx(c.dbContextKey, db)
